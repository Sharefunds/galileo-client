...
type CreateVirtualCardRequest struct {
	...
	// To load funds into the account at the time of account creation, specify the <<glossary:PAN>> or <<glossary:PRN>> of the source account for the funds. This account must be in the same program as the account being created. Pattern: PAN or PRN Example: `\"074103447228\"`
	...
	// Credit limit for card based spending control (currently only available for credit processing). Pattern: Monetary amount greater than 0. Example: `500.25`
	CreditLimit float64 `json:"creditLimit,omitempty"`
	// Spending control parameter that flags card for one purchasing transaction (currently only available for credit processing). Pattern: `Y` or `N` Example: `\"Y\"`
