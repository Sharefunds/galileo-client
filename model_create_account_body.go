...
type CreateAccountBody struct {
	...
	// Currency amount passed as a whole or decimal amount. Initial load amount on a card must be within product load limits or be the designated amount for an instant-issue card. Pattern: Positive integer or decimal number Example: `100.00`, `100`  or `100.73`
	LoadAmount float32 `json:"loadAmount,omitempty"`
	// Transaction type for the `loadAmount`. Use values from your Galileo-supplied list of load types. If no `loadType` is specified, the default type `RL` is used. Pattern: 2 characters Example: `\"RL\"`
	...
	// 0=online transaction, 1=offline transaction Pattern: `0` or `1` Example: `0`
	...
	// CIP setting override. Valid only if you are using Galileo's integrated <<glossary:CIP>> solution. * `0` or empty &mdash; Do not override the CIP setting. * `1` &mdash; Create the customer record and run CIP but do not create an account. * `2` &mdash; Create the customer record and account but do not run CIP.  Example: `\"0\"`
	CipStatus string `json:"cipStatus,omitempty"`
	...
	// Fee amount assessed by the provider. This value is passed to Galileo only for informational purposes: passing this parameter does not assess the fee. Pattern: Monetary amount greater than 0. Example: `2.50`
	ProviderAssessedFee float32 `json:"providerAssessedFee,omitempty"`
	// To load funds into the account at the time of account creation, specify the <<glossary:PAN>> or <<glossary:PRN>> of the source account for the funds. This account must be in the same program as the account being created. Pattern: PAN or PRN Example: `\"123456789012\"`
	LoadFromAccountNo string `json:"loadFromAccountNo,omitempty"`
	// The last date that a daily sweep should be performed. This parameter is valid only if account sweeping is configured for the product. Pattern: YYYY-MM-DD Example: `\"2016-01-01\"`
	SweepDate time.Time `json:"sweepDate,omitempty"`
	// The type of express mail for shipping the card associated with this account. Use the values that are supported by your emboss vendor. Pattern: `Y`, `N`, or `0-9` Example: `\"1\"`
	ExpressMail string `json:"expressMail,omitempty"`
	// Pass `1` to always ship cards to the ship-to address instead of one time only. Pattern: `1` or `0` Example: `\"1\"`
